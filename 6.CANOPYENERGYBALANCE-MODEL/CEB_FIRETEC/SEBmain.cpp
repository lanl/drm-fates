...
string trash, AMout, LD, MD, SD, FD, TTS1, TTS2, TTS3, TTS4, FS;
...
FuelMoisture = allocate_2d_double(EndTime, ArrayCount);  // =  {0};
...
for (int dn=0; dn < (NZ*NY*NX); dn++) { // LOOP TO ASSING DATA, LOCATION & LAI OF EACH CELL WITH FUEL
    if (FuelPointer[dn] > fueltol){
       ...
       // Determine LAI   
       LaiArray[ArrayIndex] = FuelPointer[dn] * DZarray[ArrayIndex] / (rhomicro * sizescale); // Consider Reading Sizescale form file. 
...
int sunresult;
int tt = 0, f = 0, z = 0, fin = 0, sumloop = 0, levelsum = 0, StartLevelSearch = 0, EndLevelSearch = 0;
      ...
      for (f=0; f < CellsPerZlevel[l]; f++){// Fuel Loop  --> Calculate Shade by shooting lines from ground to sun.  Intercepted fuel causes shade.
        ...
        if (Zloc[index] < OverZ[Zlevelcount-1]) {  // The highest level of fuel never gets shade
          for (z; z < Zlevelcount; z++){ // Looping beam up through the canopy
            ...
            /*
            Figure out a way to limite the search to a quarter of the domain.
            */
            // This limits the search in the fuel interception loop just below.  It needs to be limited way more!!!!!
            StartLevelSearch = levelsum  + CellsPerZlevel[z];
            EndLevelSearch = StartLevelSearch + CellsPerZlevel[z+1];
            fin = StartLevelSearch;
            //cout<<tt<<"  "<<index<<"  "<<z<<"  "<<fin<<"  "<<StartLevelSearch<<"  "<<EndLevelSearch<<"  "<<levelsum<<"     "<<CellsPerZlevel[z]<<"     "<<ArrayCount<<endl;
            for (fin; fin < EndLevelSearch; fin++) { // loops through fuel cells to see if loaction of traced line crosses them.  ***** Change ArrayCount and limit it to that level.
              //cout<<"         "<<fin<<endl;
...
//cout<<tt<<"  "<<index<<"   "<<fin<<"   "<<T_length<<"  X1, Y1, Z1: "<<X1<<"  "<<Y1<<"  "<<Z1<<"  X2, Y2, Z2: "<<X2<<"  "<<Y2<<"  "<<Z2<<"  DZ: "<<DZarray[fin]<<"  LAI: "<<LaiArray[fin]<<" "<<TransectLAI<<"  "<<QswFactor[tt][index]<<endl;
            ...
            //cout<<tt<<"  "<<index<<"  "<<z<<"   *#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*    "<<QswFactor[tt][index]<<endl;
            levelsum = levelsum + CellsPerZlevel[z];
          } // z Level loop
...
// METEROLOGICAL INPUT FILE
  ...
  //TimeSeires2stream<<"Tstep   RH   AirTemp  LeafTemp    ActualVP   SurfaceVP       EvLa "<<endl;
  //TimeSeires2stream.setf(ios::fixed, ios::floatfield);
  //TimeSeires2stream.precision(7);
  //TimeSeires2stream.width(11);
  //
  ...
  ofstream TimeSeires4stream;
  TTS4="LatentHeat." + strFcell + ".out.txt";
  TimeSeires4stream.open(TTS4.c_str());
  TimeSeires4stream<<"Tstep   Qe  rowaLe   Dhe   Sqig   ActualVP   SurfaceVP   Apa    EvLa "<<endl;
  TimeSeires4stream.setf(ios::fixed, ios::floatfield);
  TimeSeires4stream.precision(8);
  TimeSeires4stream.width(12);
  ...
  for (int rel=1; rel<=EndTime; rel++) { //TIME LOOP LOOP LOOP #####################
      ...
      //TimeSeires2stream<<setw(4)<<rel<<"    "<<setw(4)<<st_data.vp_air.relative_humidity<<"   "<<st_data.vp_air.temp-273.15<<"  "<<st_data.st_energy.Ts-273.15<<"  "<<st_data.vp_air.actual_vaporpressure<<"  "<<st_data.vp_surface.actual_vaporpressure<<"  "<<st_data.st_energy.EvL<<endl;
      //TimeSeires3stream<<setw(4)<<rel<<"    "<<setw(3)<<st_data.st_energy.fQswIn<<"  "<<st_data.st_energy.fQlwIn<<"  "<<st_data.st_energy.fQlwOut<<"  "<<st_data.st_energy.fQh<<"  "<<st_data.st_energy.fQe<<"  "<<st_data.st_energy.Dhe<<"  "<<st_data.st_energy.EvR<<endl;
      TimeSeires4stream<<setw(4)<<rel<<"    "<<st_data.st_energy.fQe<<"  "<<st_data.st_energy.rowaLe<<"  "<<st_data.st_energy.Dhe<<"  "<<st_data.st_energy.Sqig<<"  "<<st_data.vp_air.actual_vaporpressure<<"  "<<st_data.vp_surface.actual_vaporpressure<<"  "<<st_data.st_energy.EvL<<"        "<<st_data.vp_surface.saturated_vaporpressure<<endl;     
...
deallocate_2d_double(FuelMoisture, EndTime, ArrayCount); 
	return 0;
}
