...
/* THIS CODE WILL CALCULATE THE SUNLIGHT REACHING A CELL
...
string trash, AMout, AVS_out, Mois_out, LD, MD, IP, SD, FD, TTS1, FS;
...
ifstream InputData;
IP = "InputList.txt";
InputData.open(IP.c_str());
if (!InputData){
   cout <<"ERROR opening "<<IP<<" input file"<<endl;
} getline (InputData,trash);
getline (InputData,trash);
getline (InputData,trash);

InputData  >>spa.latitude>>spa.longitude>>spa.elevation>>spa.timezone;
InputData  >>spa.year>>spa.month>>spa.day>>spa.hour>>spa.minute>>spa.second>>EndTime;

cout<<"Input Data: lat, elvation,  year "<<spa.latitude<<"  "<<spa.elevation<<"  "<<spa.year<<"  "<<spa.month<<"  "<<EndTime<<endl<<endl;
...
for (int dn=0; dn < (NZ*NY*NX); dn++) { // LOOP TO ASSING DATA, LOCATION & LAI OF EACH CELL WITH FUEL
    ...
    if (FuelPointer[dn] > fueltol){ // BIG DOMAIN
       ...
       if (((dn/(NX*NY))%NZ) > zhold ){
          OverZ[Zlevelcount] = Zloc[ArrayIndex];
          CellsPerZlevel[Zlevelcount] = counter;
          cout<<"NEW LEVEL: "<<((dn/(NX*NY))%NZ)<<"  "<<zhold<<"  "<<Zloc[ArrayIndex]<<"  "<<OverZ[Zlevelcount]<<"  "<<CellsPerZlevel[Zlevelcount]<<endl;
          Zlevelcount++;
          counter=0;
          zhold = Zloc[ArrayIndex];
       } 
       //cout<<ArrayIndex<<"  "<<Xloc[ArrayIndex]<<"  "<<Yloc[ArrayIndex]<<"  "<<Zloc[ArrayIndex]<<"  DZarray: "<<DZarray[ArrayIndex]<<"  "<<((dn/(NX*NY))%NZ)<<"  "<<zhold<<endl;
       //if ( DZarray[ArrayIndex]>DZarray[ArrayIndex-1]){
       //   OverZ[Zlevelcount] = Zloc[ArrayIndex];
       //   CellsPerZlevel[Zlevelcount] = counter;
       //   std::cout<<"Z-level counts  "<<Zlevelcount<<"  "<<Zloc[ArrayIndex]<<"  "<<DZarray[ArrayIndex]<<"  "<<ArrayIndex<<"  "<<CellsPerZlevel[Zlevelcount-1]<<"  "<<std::endl;
...
CellsPerZlevel[Zlevelcount] = counter;
//Zlevelcount++;
cout<<"FINAL COUNTER: "<<counter<<"  "<<Zlevelcount<<endl;
//FuelData.close(); // MOCK DOMAIN
...
//    //cout<<i<<"  DZarray: "<<DZarray[i]<<"  cell_data: "<<cell_data<<"  Zlevelcount: "<<Zlevelcount<<endl;
...
for (int zl = 0; zl < Zlevelcount; zl++) {
    cout<<zl<<"   Cells Per Level: "<<CellsPerZlevel[zl]<<"  "<<OverZ[zl]<<"  "<<OverZ[Zlevelcount-2]<<endl;
}
...
for (tt=0; tt < EndTime; tt++) {
    ...
    sunresult = spa_calculate(&spa);
     ...
     if (zenith<85) { // Sun is above the horizon Shade will happen
      ...
      //cout<<"IN SHADOW LOOP: "<<l<<"  "<<CellsPerZlevel[l]<<"  "<<OverZ[Zlevelcount-1]<<"  "<<OverZ[Zlevelcount]<<"  "<<Zlevelcount<<endl;    
      for (f=0; f < CellsPerZlevel[l]; f++){// Fuel Loop  --> Calculate Shade by shooting lines from ground to sun.  Intercepted fuel causes shade.
        ...
        //cout<<f<<"  "<<Zloc[index]<<"  "<<OverZ[Zlevelcount-1]<<endl;
        if (Zloc[index] < OverZ[Zlevelcount-1]) {  // The highest level of fuel never gets shade

// Find point that ray exits cell
// ADD LOOP HERE FOR RAY RESOLUTION ##################################
          ang_.x_raySt = 0.75*DX, ang_.y_raySt = 0.75*DY, ang_.z_raySt = 0, ang_.DZ = DZarray[index]; // Determines Ray Starting Point NEED TO PUT IN LOOP TO INCREASE RESOLUTION
          in = 0;
          // MAP TO LARGE DOMAIN
          ID = Address[index];
          //ID = DomainMAP[ID];
...
//}}
          ...
          //if (QswFactor[tt][index]>0.05){cout<<"Shade Factor Above 0:  "<<QswFactor[tt][index]<<endl;}
...
Amanzistream<<"TIME   Ts     ShortWaveIn,  ShortWaveShade,   DailyTotal"<<endl;
double CellVol = 0.0, QswInital = 0.0;
...
// METEROLOGICAL INPUT FILE
  ...
  for (int rel=1; rel<=EndTime; rel++) { //TIME LOOP LOOP LOOP #####################
      ...
      //// Run SEB ****** NO NEED TO RUN SURFACE ENERGY BALANCE ##############        
      //SurfaceEnergyBalance::LeafEnergyBalance(st_data);

      TIME = TIME + (Dt/60);  // Time in Days

      Amanzistream<<setw(4)<<rel<<"  "<<Xloc[f]<<"  "<<Yloc[f]<<"  "<<Zloc[f] <<setw(5)<<QswInital<<"   "<<QswIn<<"  "<<endl;
      ...
      //st_data.st_energy.fQswIn=0.0, st_data.st_energy.fQlwIn=0.0, st_data.st_energy.fQlwOut=0.0; 
      //st_data.st_energy.fQe=0.0, st_data.st_energy.fQh=0.0, st_data.st_energy.varvar=0.0;

  }//  END TIME LOOP LOOP LOOP LOOP #####################

  //cout<<"FuelPointer, f:  "<<FuelPointer[Address[f]]<<"  "<<f<<endl;
