...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      !!!! DRM TreeTracker Stuff - Ask ADAM, Even though he won't remember!!! ~AA
      integer cellnum, cn
      ...
      ! Open tree tracking file:  AA~ 3/20/22
     ...
     +    treespec(sum(ntrees)),rhospecies(nx,ny,nz,ntspecies))
      ...
      do i=1,ntspecies
        ...
        do j=1,ntrees(i)
          cellnum=0   !!! ~AA 3/20/22
          tfueltot=0  !!! ~AA 3/20/22        
     ...
     +                        (yloc-ytest)**2.)/canopydiameter**2.)/(canopytop-canopybot) ! Contribution of one subcell to overall bulk density; 
             ...
             if (sum(rhoftemp).gt.0) then !! ~AA
               cellnum = cellnum+1    !! ~AA
               do ift=1,tfuelbins
     ...
     +                  (trhof(ift_index,ii_real,jj_real,kk)+rhoftemp(ift))
                 ...
                 enddo
                 cellid(cellnum) = (ii_real+((jj_real - 1)*nx)+((kk - 1)*nx*ny)) !! ~AA
                 cellfuel(cellnum) = sum(rhoftemp) * dx * dy * (zheight(ii_real,jj_real,kk+1)-zheight(ii_real,jj_real,kk)) !! ~AA
          ...
          enddo
          do cn=1, cellnum   !! ~ AA
             cellfuel(cn) = cellfuel(cn)/tfueltot    !! ~ AA
