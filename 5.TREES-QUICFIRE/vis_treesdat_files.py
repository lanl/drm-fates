...
def plotVertical(fig,axs,arr,title,X,Z):
    #sp1 = axs.pcolormesh(X[:,int(Ny/2),:],Z[:,int(Ny/2),:],arr[:,int(Ny/2),:],cmap='jet',shading='auto')
    sp1 = axs.pcolormesh(X[:,int(Ny/2),:],Z[:,int(Ny/2),:],np.sum(arr,axis=1),cmap='jet',shading='auto')
    cbar = fig.colorbar(sp1, ax=axs)
    #cbar.ax.set_ylabel(ylabel, rotation=270)
    axs.set_title(title)
    axs.set_xlabel('X [m]')
...
for n in range(nfuel+1):
    ...
    if (rows!=1):
        ...
        inpaxs = axs[n]
    plotVertical(fig,inpaxs,arr[:,:,0:22],t,XI[:,:,0:22]+Nx,ZI[:,:,0:22])  
#Hide unused subplots
for nn in range(nfuel+1,rows*cols):
    axs[int(np.floor((nn)/cols)),((nn)%cols)].axis('off')
#figure title    
