...
class LLM:
    ...
    def instantiate(self,readf):
		...
		if readf:
			f = open('data/LP_ht.txt') # from 100 year simulation (output from a previous version)
			f1 = open('data/LP_count.txt')
			f2 = open('data/LP_age.txt')
			#f3 = open('data/lit_WG.txt')
			f4 = open('data/HW_ht.txt')   # used random HW sprouts, see beg_stand()
			f5 = open('data/HW_count.txt')
			f6 = open('data/HW_age.txt')
...
##        print 'no. of starting LLP trees', no_old_ht
        ...
        #print 'shape:',np.shape(self.old_htHW)
        for row in range(self.dim):
            for col in range(self.dim):
                self.rand_calc()
    ...
    def seed_prodLP(self):
        ...
        if self.mast_yr == 'yes':
            ...
            if self.seed_int == 1:
                ...
                cones_tree2 = random.randint(40,50) # adult cones/tree
                
            elif self.seed_int == 2:
                cones_tree1 = random.randint(20,30)
                cones_tree2 = random.randint(50,70)
                
            elif self.seed_int == 3:
                cones_tree1 = random.randint(30,50)
                cones_tree2 = random.randint(70,90)
                
            else: # = 4
                cones_tree1 = random.randint(50,70)
                cones_tree2 = random.randint(90,125)
                
        else: # normal seed year
            
            if self.seed_int == 1:
                cones_tree1 = random.randint(0,5) # subadult cones/tree
                cones_tree2 = random.randint(0,10) # adult cones/tree
                
            elif self.seed_int == 2:
                cones_tree1 = random.randint(5,10)
                cones_tree2 = random.randint(10,20)
                
            elif self.seed_int == 3:
                cones_tree1 = random.randint(10,15)
                cones_tree2 = random.randint(20,30)
                
            else: # = 4
                cones_tree1 = random.randint(15,20)
                cones_tree2 = random.randint(30,40)
    ...
    def fire_spread_flag(self):
        ...
        burndist = self.dim/2 + 1  #random.randint(1,self.dim/2+1) 
...
##            print ''
            ...
            #EJ modified
            [lb, ub, bn, s] = np.loadtxt('HVI_inputs.txt', delimiter=',', usecols=(1,2,3,6), skiprows=7 ,unpack=True)
            self.age_sc.append(hvi.score_subset(self.old_age, lb[0], ub[0], bn[0], smooth=s[0]))            
            self.hw_sc.append(hvi.score_subset(self.old_ht, lb[1], ub[1], bn[1],smooth=s[1]))
            
            #nn=self.dim
            #dbh=hvi.age2dbh(self.old_age.reshape(nn*nn))
            #self.dbh_sc.append(hvi.score_subset(dbh.reshape(nn,nn), lb[2], ub[2], bn[2],smooth=s[2]))
            self.ageHW_sc.append(hvi.score_subset(self.old_ageHW, lb[3], ub[3], bn[3], smooth=s[3]))            
            self.hwHW_sc.append(hvi.score_subset(self.old_htHW, lb[4], ub[4], bn[4],smooth=s[4]))
            
            self.gt_sc.append(hvi.prob_of_use_gt(self.old_age, self.old_LPcount, self.old_htHW, self.old_HWcount, self.tree_mature_age))
            self.sq_sc.append(hvi.prob_of_use_sq(self.old_age, self.old_LPcount, self.old_ageHW, self.old_HWcount, self.tree_mature_age, nsize=5, msize=5))

            if self.verbose:
		print 'age score:',round(self.age_sc[-1],2),lb[0], ub[0], bn[0]
		print 'height score:' , round(self.hw_sc[-1],2),lb[1], ub[1], bn[1]
#		print 'dbh score:' , round(self.dbh_sc[-1],2),lb[2], ub[2], bn[2]
		print 'ageHW score:',round(self.ageHW_sc[-1],2),lb[3], ub[3], bn[3]
		print 'height score:' , round(self.hwHW_sc[-1], 2),lb[4], ub[4], bn[4]
		print 'squirrel score:',round(self.sq_sc[-1],2)
		print 'GT score:',round(self.gt_sc[-1],2)
		print '----------------'
		print ''
				
            ###self.dbh_sc.append(hvi.score_subset(np.reshape(dummy, (self.dim,self.dim)), lb[2], ub[2], bn[2], smooth=2))
            #print 'dbh score:',self.dbh_sc[-1]
            ###self.ba_sc.append(hvi.score_subset(np.reshape(dummy, (self.dim,self.dim)), lb[3], ub[3], bn[3], smooth=s[3]))
            #print 'ba score:',self.ba_sc[-1]
            #print 'fire_prob',self.fire_prob
##            print 'dormant years ', self.dorm_tag
##            print ' age lp ', self.old_age
##            print ' ht lp ', self.old_ht
            #print ' end of year ', tim
