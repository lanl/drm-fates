...
def toLandis(lp):
    ...
    postfire.to_csv(
        os.path.join(
            lp.landis_path, "Treelist_postfire_cycle" + str(lp.cycle) + ".csv"
        ),
        index=False,
    )
    ...
    community_input_file = merge_cohorts(
        postfire_cohorts,
        lp.landis_path,
        "community-input-file-" + str(lp.year_prev) + ".csv",
        lp.cycle,
...
def postfire_treelist(postfire_path, treelist_path, cycle):
    ...
    newtreelist = pd.read_csv(
        os.path.join(postfire_path, "AfterFireTrees." + str(cycle) + ".txt"),
        sep=" ",
        header=None,
    )
    ...
    treelist_alldata = pd.read_csv(
        os.path.join(
            treelist_path, "Treelist_postlandis_cycle" + str(cycle - 1) + ".csv"
        )
    )
    newtreelist.columns = [
        "SPID",
    ...
    newtreelist = newtreelist[
        ["SPID", "X", "Y"]
    ]  # use only columns needed to match to old treelist
    ## Assign attributes from pre-QF treelist to the remaining post-QF trees
    newtreelist_alldata = newtreelist.merge(
        treelist_alldata, how="left", on=["SPID", "X", "Y"]
    )
    return newtreelist_alldata


def treelist_to_cohorts(x, L2_res, spec_rename):
    community_input_file = x.groupby(
        ["MapCode", "SPECIES_SYMBOL", "AGE"], as_index=False
    ).sum("AGB_g")
    community_input_file = community_input_file.assign(
        CohortBiomass=lambda x: x.AGB_g / L2_res**2
    )
    community_input_file = community_input_file[
        ["MapCode", "SPECIES_SYMBOL", "AGE", "CohortBiomass"]
    ]
    community_input_file = community_input_file.rename(
        {"SPECIES_SYMBOL": "SpeciesName", "AGE": "CohortAge"}, axis="columns"
    )
...
def merge_cohorts(postfire, path, CIF_in, cycle, OC_in):
    prefire = pd.read_csv(
        os.path.join(path, "Treelist_postlandis_cycle" + str(cycle - 1) + ".csv")
    )
    ...
    if np.array_equal(burndomain_mc, uncropped_mc):
        ...
        outside_burndomain = prefire_uncropped[
            ~prefire_uncropped["MapCode"].isin(burndomain_mc)
        ]
        postfire_landis = pd.concat([outside_burndomain, postfire_all])
    postfire_landis = postfire_landis[
...
def write_IC(IC, path, cycle):
    with open(
        os.path.join(path, "postfireIC_cycle" + str(cycle) + ".txt"), "w"
    ) as file:
                ...
                for j in IC_mc["SpeciesName"].unique():
                    if j == None:
                        ...
                        for k in range(0, IC_mc_sp.shape[0]):
                            file.write(
                                "{} ({}) ".format(
                                    int(IC_mc_sp.loc[k, "CohortAge"]),
                                    int(math.ceil(IC_mc_sp.loc[k, "CohortBiomass"])),
                                )
                            )
                        file.write("\n")
            file.write("\n")
...
def replace_fuels(
    OG_PATH, landis_path, cycle, coarseroots_map, IC_map, L2_res, year_prev, crop_domain
):
    litter_arr = np.loadtxt(
        os.path.join(
            OG_PATH,
            "1.LANDIS-MODEL",
            "FM2VDM",
            "AfterFireLitter." + str(cycle) + ".txt",
        )
    )
    ...
    with rio.open(
        os.path.join(
            landis_path, "output-community-cycle" + str(cycle - 1) + "_cropped.tif"
        ),
        "r+",
    ) as IC:
        with rio.open(
    ...
    with rio.open(
        os.path.join(
            landis_path, "NECN", "SurfaceLitterBiomass-" + str(year_prev) + ".img"
        ),
        "r+",
    ) as slb:
        slb_arr = slb.read(1)
    ...
    if cycle == 1:
        ...
        os.rename(
            os.path.join(landis_path, coarseroots_map),
