...
def Landis(lp):
    if lp.crop_domain:
        ...
        OC_path = os.path.join(
            lp.landis_path, "output-community-" + str(lp.year) + ".img"
        )
        litter_path = os.path.join(
            lp.landis_path, "NECN", "SurfaceLitterBiomass-" + str(lp.year) + ".img"
        )
        needles_path = os.path.join(
            lp.landis_path, "NECN", "ConiferNeedleBiomass-" + str(lp.year) + ".img"
        )

        if lp.spinup:
            #### Create burn domain that lines up with landis grid cells
            IC_path = os.path.join(lp.landis_path, lp.IC_map)
            shutil.copy(IC_path, os.path.join(lp.landis_path, "IC_original.tif"))
            ...
            if os.path.exists(os.path.join(OG_PATH, "Shapefiles", "burn_domain.shp")):
                with fiona.open(
                    os.path.join(OG_PATH, "Shapefiles", "burn_domain.shp")
                ) as shapefile:
                ...
                burn_domain = gpd.GeoDataFrame(
                    {"col1": ["name"]}, geometry=[domain_poly], crs=IC_epsg
                )
                ...
                with rio.open(IC_path, "r+") as landis_rast:
                    ...
                    geometry = [
                        Point(landis_rast.xy(x, y)[0], landis_rast.xy(x, y)[1])
                ...
                N, S, E, W = [
                    domain_pts.geometry.y.max() + lp.L2_res / 2,
                ...
                domain_poly = Polygon([(W, S), (W, N), (E, N), (E, S), (W, S)])
                new_domain = gpd.GeoDataFrame(
                    {"col1": ["name"]}, geometry=[domain_poly], crs=domain_pts.crs
                )

                ## Write to file
                new_domain.to_file(
                    os.path.join(os.path.join(OG_PATH, "Shapefiles", "burn_domain.shp"))
                )
                with fiona.open(
                    os.path.join(os.path.join(OG_PATH, "Shapefiles", "burn_domain.shp"))
                ) as shapefile:
        ...
        if lp.spinup == False:
            IC_path = os.path.join(lp.landis_path, "IC_original.tif")

        ## Crop the output community raster
        OC_tif = georeference(
            IC_path,
            OC_path,
            "output-community-" + str(lp.year) + ".tif",
            lp.landis_path,
        )
        crop_raster(
            OC_tif,
        ...
        ## Crop fuels rasters
        litter_tif = georeference(
            IC_path,
            litter_path,
            "SurfaceLitterBiomass-" + str(lp.year) + ".tif",
            lp.landis_path,
        )
        needles_tif = georeference(
            IC_path,
            needles_path,
            "ConiferNeedleBiomass-" + str(lp.year) + ".tif",
            lp.landis_path,
        )
        crop_raster(
            litter_tif,
        ...
        cif = pd.read_csv(
            os.path.join(
                lp.landis_path, "community-input-file-" + str(lp.year) + ".csv"
            )
        )
        ...
        src = [
            os.path.join(lp.landis_path, "output-community-" + str(lp.year) + ".img"),
            os.path.join(
                lp.landis_path, "community-input-file-" + str(lp.year) + ".csv"
            ),
        ]
...
def georeference(IC_path, img_path, tif_name, landis_path):
    with rio.open(IC_path, "r+") as IC:
        with rio.open(img_path, "r+") as img:
            ...
            img.crs = IC.crs
            with rio.open(
                os.path.join(landis_path, tif_name),
                mode="w",
                height=img.height,
                width=img.width,
                count=1,
