...
def postfire_fuels(ft: FuelsTracker, tp, lp):
            ...
            with open(os.path.join(ft.scorch_path, ft.treelist), "r") as tl:
                ...
                for tt, tl in treezip:
                    ...
                    for cell in range(cellnum):
                        ...
                        totfuel += newfuel  # sum the total amount of remaininf fuel for that tree
...
def FuelChange(path, nx, ny, nz):
    sim = SimulationOutputs(os.path.join(path, "Output"), nz, ny, nx)
    dens = sim.get_output("fuels-dens")
    dens_init = dens.to_numpy(timestep=0)
    dens_final = dens.to_numpy(timestep=len(dens.times) - 1)
    Drhofp = np.divide(dens_final[0, :, :, :], dens_init[0, :, :, :])
    Drhofp[np.isnan(Drhofp)] = 1
    fuel_change_path = os.path.join(path, "PercentFuelChange.txt")
    print("Writing PercentFuelChange.txt")
    with open(fuel_change_path, "w") as pfc:
        for locz in range(nz):
            for locy in range(ny):
                for locx in range(nx):
                    PercRhofChange = Drhofp[locz, locy, locx]
