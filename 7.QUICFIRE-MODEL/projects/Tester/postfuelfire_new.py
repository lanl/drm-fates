...
def plot_fuelheight(qf, ground_level_fuel_height, save_dir, img_specs):
    ...
    pylab.imshow(ground_level_fuel_height, cmap=my_cmap, interpolation='none', origin='lower',
                 extent=qf.horizontal_extent, vmin=myvmin, vmax=myvmax)
    cbar = pylab.colorbar()
    ...
    pylab.savefig(save_dir + os.sep + 'fuel_height_ground_level.png')
    pylab.close()
...
def plot_percmassburnt(qf, myextent, plotvar, fuel_dens, ystr, savestr, save_dir, img_specs):
    ...
    for i in range(0, qf.ntimes):
        ...
        if len(k_loc) > 0:
            i_loc = k_loc[1]
            j_loc = k_loc[0]
            currval[tuple([j_loc, i_loc])] = loc_var[tuple([j_loc, i_loc])]
...
def plot_outputs(qu, qf, ignitions, flags, fb):
    ...
    # ADAM CHANGES
    Drhofp=np.divide(fuel_dens[int(qf.time[-1]/100)][:,:,:],fuel_dens[0][:,:,:])
    Drhofp[np.isnan(Drhofp)]=1
    PercRhofChange = 0.0
    print("ADAM ATCHLEY",qf.time,qf.nx,qf.ny,qf.nz)
    for i in range(qf.nz):
      print(i, fuel_dens[0][55,51,i])
      print(i, Drhofp[55,51,i])
    ...
    pfc.close() 
    #fuel_dens[qf.time][df.nx,qf.ny,qf.nz]
    ...
    ground_level_fuel_height = read_ground_fuel_height(qf)
    plot_fuelheight(qf, ground_level_fuel_height, save_dir, img_specs)
    ...
    if flags.fuel_density == 1:
        for iplane in (1,2,5,8,10,12):
            if iplane <= qf.nz:        
                print("Plane ", iplane, "  - fuel mass")
                fuel_dens = read_fireca_field("fuels-dens-", qf.ntimes, qf.time, qf, 0)
                print("ADAM", len(fuel_dens), qf.ntimes, qf.time)
                print("ADAM", len(fuel_dens[1]))
                plot_2d_field(False, qf, iplane, fuel_dens, "Fuel density [kg/m^3]", "fuel_dens",
                        [0., np.amax(fuel_dens[0][::1, ::1, iplane-1], axis=None)], save_dir, img_specs)
                del fuel_dens
    ...
    if flags.emissions == 1:
        ...
        for it in range(0, qf.ntimes_ave):
            emiss[it] = np.log10(emiss[it], where=(emiss[it] != 0))
            temp = emiss[it][emiss[it] != 0]
            if np.size(temp) > 0:
                minval = min(minval, np.min(temp))
            maxval = max(maxval, np.max(emiss[it]))
        minval = math.floor(minval)
        maxval = math.ceil(maxval)
        plot_2d_field(True, qf, plane, emiss, "Soot (log10) [g]", "pm_emissions",
        ...
        minval = math.floor(minval)
        maxval = math.ceil(maxval)
        plot_2d_field(True, qf, plane, emiss, "CO (log10) [g]", "co_emissions",
                      [minval, maxval], save_dir, img_specs)
        del emiss
    ...
    if flags.thermal_rad == 1:
        ...
        plot_2d_field(True, qf, plane, conv, "Convective heat to human [kW/m^2 skin]", "conv_heat",
                      [], save_dir, img_specs)
        del conv
...
def read_ground_fuel_height(qf):
    fuel_height = np.zeros((qf.ny, qf.nx))

    fid = open_file("groundfuelheight.bin", 'rb')
    # Read header
    np.fromfile(fid, dtype=np.int32, count=1)
    t = np.fromfile(fid, dtype=np.float32, count=qf.nx * qf.ny)
    fuel_height[::1, ::1] = np.reshape(t, (qf.ny, qf.nx))
    fid.close()

    return fuel_height


def read_fireca_field(filestr, ntimes, times, qf, is_3d):
    outvar = []
    ...
    for i in range(0, ntimes):
        ...
        if is_3d == 0:
            ...
            index = tuple([qf.indexing.ijk[::1, 1], qf.indexing.ijk[::1, 0], qf.indexing.ijk[::1, 2]])
            temp[index] = var
        else:
            temp = np.zeros((qf.ny, qf.nx, nvert))
...
def read_times(fid, qu, qf):
    ...
    qf.dt_print_ave = int(get_line(fid, 1) * qf.dt_print)

    # Print time for averaged QUIC variables
    qu.dt_print_ave = int(get_line(fid, 1) * qu.dt_print)
...
def read_fire_grid_indexing(qf):
    fid = open_file('fire_indexes.bin', 'rb')
    np.fromfile(fid, dtype=np.int32, count=1)
...
def export_vtk(qf, qu, flags):
    ...
    print("ADAM", len(fuel_dens), len(fuel_dens[1]))
